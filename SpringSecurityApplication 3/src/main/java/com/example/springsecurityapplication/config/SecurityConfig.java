...
//                .antMatchers("/user").hasAnyRole("USER")
                ...
                .antMatchers("/auth/login", "/error", "/auth/registration", "/product", "/product/info/{id}", "/img/**", "/product/search","product/add","/css/**","/js/**").permitAll()
...
//                .anyRequest().authenticated()
                ...
                // Указываем на какой url адрес будут отправляться данные с формы.
                // Нам уже не нужно будет создавать метод в контроллере и обрабатывать
                // данные с формы. Мы задали url по умолчанию, который позволяет
                // обрабатывать форму аутентификации в спринг секьюрити. Спринг
                // секьюрити будет ждать логин и пароль с формы и затем сверить
                // их с данными в БД
...
@Override
    public void configure(WebSecurity web) throws Exception {
        web
                .ignoring()
                .antMatchers("/resources/**", "/static/**", "/css/**","/scss/** ","/js/**","/img/**","/icon/**");
    }
    @Bean
